function combine_simulation_figures()
...
savefilename = fullfile(simulation_result_subfolder, ['samples' num2str(samples(1)) '-' num2str(samples(end)) '.ps']);
...
for sampleID = samples
    ...
    simulationresultfileheader = fullfile(simulation_result_subfolder2, [link '_k' num2str(embed_width) '_tau' num2str(embed_step) '_K' num2str(K) '_kGMM' num2str(kGMM) 'sample' num2str(sampleID)]);
