function draw_connection_gridmaps_eachsample_separate(UGE_TASK_ID_text)
...
global samples common_data_folder connection_strength_folder globalvalidweightarray ...
    ...
    save_each_sample_tif save_all_samples_single_pdf embed_size embed_step embed_width link models_folder kGMM ...
...
samples = 1:24;
samplei = str2num(UGE_TASK_ID_text);
...
savefilenameheader = ['sample' num2str(sampleID) '_pcolor'];
...
savefilenameheader2 = ['sample' num2str(sampleID) '_brickwork'];
...
savefilenameheader3 = ['sample' num2str(sampleID) '_lagweights'];
...
if save_all_samples_single_pdf && exist(savefilenamesingle, 'file')
...
%for samplei = 1:length(samples)
 sampleID = samples(samplei);
    ...
    if save_all_samples_single_pdf
...
if ~strcmp(computer, 'MACI64') && save_all_samples_single_pdf  % Mac does not support use of ps2pdf like this
    ...
    for rowi = 1:nrow
        for coli = 1:ncol
            ...
            if rowi == 1
                ...
                if ncol > 80
                    ax(figcount).FontSize = 4;
                elseif ncol > 60
                    ax(figcount).FontSize = 6;
                elseif ncol > 40
                    ax(figcount).FontSize = 8;
                else
                    ax(figcount).FontSize = 10;
                end
            end
            if coli == 1
                if nrow > 100
                ...
                set(hYLabel,'rotation',0,'VerticalAlignment','middle','HorizontalAlignment','right','FontSize',fontsize)
            end
        end
    end
    ...
    for rowi = 1:nrow
        for coli = 1:ncol
            ...
            if rowi == 1
                ...
                ax(figcount).Title.VerticalAlignment = 'middle';
                ax(figcount).Title.HorizontalAlignment = 'left';
                % ax(figcount).TitleFontSizeMultiplier = xx;
                if ncol > 80
                    ax(figcount).FontSize = 4;
                elseif ncol > 60
                    ax(figcount).FontSize = 6;
...
if ~strcmp(computer, 'MACI64') && save_all_samples_single_pdf
    system(['ps2pdf ' savefilenamesingle]);
    pdffilename = [savefilenameheader '_single.pdf'];
    disp(['mv ' pdffilename ' ' connection_strength_folder])
    system(['mv ' pdffilename ' ' connection_strength_folder]);
