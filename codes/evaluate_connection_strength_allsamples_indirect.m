...
function evaluate_connection_strength_allsamples_indirect()
...
for Ki = 1:length(Ks)
    ...
    for samplei = 1:length(samples)
        ...
        revoutperm = 1:Mt;
        revoutperm(outperm) = 1:Mt;
        globaloutpermmat(samplei,cellg) = (revoutperm-0.5)/Mt;
    ...
    meanweightmatrix(tweightmatrix>0.05) = NaN;   % 0.01
    meanweightmatrix1 = cat(2, NaN(Mg+1,1), meanweightmatrix);
    
    save(fullfile(metadata_folder, 'globalweightmatrix.mat'),'globalweightmatrix', 'globalweightmatrix1','meanweightmatrix','meanweightmatrix1','tweightmatrix','globaloutperm')


    %load(fullfile(metadata_folder, 'globalweightmatrix.mat'),'globalweightmatrix', 'globalweightmatrix1','meanweightmatrix','meanweightmatrix1','tweightmatrix','globaloutperm')
    
    %disp(globalNames(globaloutperm))
    
    globaloutperm1 = [1,globaloutperm+1];
    lab=cellstr(globalNames1(globaloutperm1));
    labnum = cell(1,Mg+1);
    for celli =1:Mg+1
        labnum{celli} = [strrep(lab{celli}, '_', '\_')]; %  ' ' num2str(celli)
    end
    gradientmatrix_plot(meanweightmatrix1(globaloutperm1,globaloutperm1), 'allsamples', '', fullfile(results_folder, ['all_network_K' num2str(K) '_allsamples']), Mg+1, labnum);%,sampleID, K, lambda);
    gradientmatrix_plot(meanweightmatrix1(globaloutperm1,globaloutperm1)', 'allsamples', '', fullfile(results_folder, ['all_network_K' num2str(K) '_allsamples_rev']), Mg+1, labnum);%,sampleID, K, lambda);
    ...
    groupC_names = globalNames(globaloutperm(1:30));
    groupD_names = {'AIBL','AIBR','RIML','RIMR','AVEL','AVER','AVAL','AVAR'};
    ...
    for i = 1:length(globaloutperm)
        if any(strcmp(globalNames{globaloutperm(i)},groupC_names))
            outpermC = [outpermC, globaloutperm(i)+1];
        end
        if any(strcmp(globalNames{globaloutperm(i)},groupD_names))
            outpermD = [outpermD, globaloutperm(i)+1];
        end
    ...
    draw_graph(meanweightmatrix1, outpermC, globalNames1, zeros(1,nc),  linspace(-1,1,nc), ['All samples, Group C'], fullfile(results_folder, ['C_network_K' num2str(K) '_allsamples']))
    draw_graph(meanweightmatrix1, outpermD, globalNames1, [-0.5,0.5,-0.3,0.3,-0.7,0.7,-0.5,0.5],  [1,1,0,0,0,0,-1,-1], ['All samples, Group D'], fullfile(results_folder, ['D_network_K' num2str(K) '_allsamples']))
