...
function evaluate_connection_strength_indirect_direct(UGE_TASK_ID_text) % sampleID
% originally ver102_gradient/evaluate_connection_strength5_flip.m
% gradient at 17:30 was used and simply averaged.

% distinguish direct link and indirect link in color
...
results_folder = fullfile(project_folder, ['connection_strength/' link '_direct']);
...
for Ki = 1:length(Ks)
    ...
    saltcon = NaN(1,Mt);
    for targeti = 1:Mt
        cellc(targeti) = find(strcmp(conNames, targetcellnames{targeti}));
        saltcon(targeti) = any(strcmp(saltsensors, targetcellnames{targeti}));
    end
    multiconmatrix1 = cat(2, zeros(Mt+1,1), cat(1, reshape(saltcon,1,Mt), multiconmatrix(cellc,cellc)));

    draw_graph(weightmatrix1, multiconmatrix1, outpermAB, targetcellnames1, [-ones(1,na), ones(1,nb)], zeros(1,na+nb), ['Sample ' num2str(sampleID) ', Group A/B'], fullfile(results_folder, ['AB_network_K' num2str(K) '_sample' num2str(sampleID)]))
    draw_graph(weightmatrix1, multiconmatrix1, outpermSAB, targetcellnames1, [zeros(1,ns), -ones(1,na), ones(1,nb)], [ones(1,ns) -ones(1,na+nb)], ['Sample ' num2str(sampleID) ', Sensory-Group A/B'], fullfile(results_folder, ['SAB_network_K' num2str(K) '_sample' num2str(sampleID)]))
    draw_graph(weightmatrix1, multiconmatrix1, outpermNSAB, targetcellnames1, [0, zeros(1,ns), -ones(1,na), ones(1,nb)], [1.5, ones(1,ns) -ones(1,na+nb)], ['Sample ' num2str(sampleID) ', Salt-Sensory-Group A/B'], fullfile(results_folder, ['NSAB_network_K' num2str(K) '_sample' num2str(sampleID)]))
    
end
end


function draw_graph(weightmatrix1, multiconmatrix1, outpermX, targetcellnames1, xstarts, ystarts, title, fileheader)
...
multiconmatrixX = multiconmatrix1(outpermX, outpermX);
...
gE = table2array(graph.Edges);
from = gE(:,1);
to = gE(:,2);
Direct = zeros(nedges,1);
for i = 1:nedges
   if multiconmatrixX(from(i),to(i)) == 1
       Direct(i) = 1;
   end
end
...
Ecolor(graphEdgeWeight>0 & Direct==1,2)=1-half_sigmoid(graphEdgeWeight(graphEdgeWeight>0 & Direct==1).*30); % magenta if Direct
Ecolor(graphEdgeWeight>0 & Direct==0,3)=1-half_sigmoid(graphEdgeWeight(graphEdgeWeight>0 & Direct==0).*30); % gold if Indirect
Ecolor(graphEdgeWeight>0 & Direct==0,2)=1-half_sigmoid(graphEdgeWeight(graphEdgeWeight>0 & Direct==0).*30)*0.4; % gold if Indirect
Ecolor(graphEdgeWeight>0 & Direct==0,1)=1-half_sigmoid(graphEdgeWeight(graphEdgeWeight>0 & Direct==0).*30)*0.1; % gold if Indirect
Ecolor(graphEdgeWeight<0,1)=1-half_sigmoid(-graphEdgeWeight(graphEdgeWeight<0).*30); % 
Ecolor(graphEdgeWeight<0 & Direct==0,2)=1-half_sigmoid(-graphEdgeWeight(graphEdgeWeight<0 & Direct==0).*30); % blue: indirect link
Ecolor(graphEdgeWeight<0 & Direct==1,3)=1-half_sigmoid(-graphEdgeWeight(graphEdgeWeight<0 & Direct==1).*30); % green: direct link
...
h.NodeLabel = '';
%nl = targetcellnames1(outperm(1:ncells));
nl = targetcellnames1(outpermX);
xd = get(h, 'XData');
yd = get(h, 'YData');
