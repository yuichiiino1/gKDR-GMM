...
evaluation_result_subfolder = fullfile(evaluation_result_folder, ['results_evaluate_models_K_' link]);
...
figure
cormatchmean = squeeze(nanmean(cormatch,1));
plot(Ks,mean(cormatchmean,1),'-o','LineWidth',3)
...
figure
cormatchmean = squeeze(nanmean(cormatch,1));
errorbar(Ks,squeeze(nanmean(cormatchmean,1)),squeeze(nanstd(cormatchmean,0,1))./sqrt(max(1,squeeze(sum(~isnan(cormatchmean),1)-1))))
%errorbar(mean(cormatchmean,1),std(cormatchmean,1))
hold on
plot(Ks, mean(cormatchmean,1),'-ob','LineWidth',3)
xlabel('K')
ylabel('correation mean +/- SEM allsamples')
title('Effect of K, averaged')
xlim([0.5 length(Ks)+0.5])
saveas(gcf,[resultfiletitle1 'hyouka_allmean_K_error' '.tif']);
saveas(gcf,[resultfiletitle1 'hyouka_allmean_K_error' '.fig']);
