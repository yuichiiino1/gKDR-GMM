...
samples = 1:2; %4;  % calculate sample mean and each sample
Ks = 4; %3:5;       % K and kGMM survey
kGMMs = 2; % K4kGMM1, K5kGMM1 are missing
freerun_repeat = 3;
links = {'direct','indirect'};
...
for samplei = 1:length(samples)
    ...
    for kGMM = kGMMs
        for Ki = 1:length(Ks)
            ...
            rscorematrixlinks = NaN(Mt, Mt, freerun_repeat, length(links));
            ...
            for linki = 1:2
                ...
                for testi = 1:freerun_repeat%freerunの回数
                    ...
                    if size(real_predict, 2) <= 1
                        ...
                        rscorematrix = (rmatrix1-rmatrix2).^2;
                        
                        rscorematrixlinks(:,:,testi, linki) = rscorematrix;
                        
                        %load([resultfiletitle1 'cormatch.mat'],'cormatch'); %freerun_repeat,length(samples),length(Ks),length(kGMMs)
                        
                        %cormatchlinks = cat(5,cormatchlinks, cormatch);
                    end
                end
            end
            
            rscorematrixdiff = squeeze( rscorematrixlinks(:,:,:,1) - rscorematrixlinks(:,:,:,2) );
            rscorematrixdiffmean = squeeze(nanmean(rscorematrixdiff, 3));
            
            rneurondiff = squeeze(sum(rscorematrixdiffmean, 1));
            
            [rneuronsort,rneuronorder] = sort(rneurondiff, 'descend');
            rneuronnames = targetcellnames(outperm(rneuronorder));
            
            save(fullfile(evaluation_result_folder,['sample',num2str(sampleID),'_K',num2str(K),'_kGMMs',num2str(kGMMs), '_rdiff.mat']),'rneurondiff','rneuronsort','rneuronorder','rneuronnames','rscorematrixdiff','outperm')
...
%%%%%% Difference matrix hearmap (locally generated) %%%%%

%{
load('evaluation_results/sample1_K4_kGMMs2_rdiff.mat')
figure;
colormap jet(64);
imagesc(rscorematrixdiffmean(rneuronorder,rneuronorder));
colorbar
%}
...
% cormatchlinks:
% freerun_repeat,length(samples),length(Ks),length(kGMMs),length(links)
%cormatchlinksmean = squeeze(nanmean(cormatch,1)); % mean of testi's  (samplei,Ki,kGMM,2)
%cormatchlinksdiff = squeeze( cormatchlinksmean(:,:,:,:,2) - cormatchlinksmean(:,:,:,:,1) );
